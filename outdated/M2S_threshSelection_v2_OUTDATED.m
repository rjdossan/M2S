...
% [ap1,ap2] = M2S_threshSelection_v2(Residuals_X(:,1), 'outliers', 0.01, 1)
function [S_result,percentileThresh] = M2S_threshSelection_v2(S, methodType, pAlpha_or_nrMad, plotOrNot)
...
if strcmp(methodType,'outliers')
    ...
    chiprobability= NaN(length(S_areaLimits),10);
...
%         h(f).NumBins = round(percentForNbins*length(h(f).S_used));
        h(f).NumBins = 15;
        [chi2_crit,chi2_prob,chi2_df] = M2S_gofTestChi2(h(f).S_used,df,h(f).NumBins,1);
        chi2_crit_all(f,df) = chi2_crit;
        chi2_prob_all(f,df) = chi2_prob;
        chi2_df_all(f,df) = chi2_df;
        disp('here')
        close 
        
%          S_counts = histcounts(h(f).S_used,20);
%      [chi_h(f,df),chi_p(f,df),chi_stats{f,df}] = chi2gof(h(f).S_used,'cdf',@(x)chi2cdf(x, h(f).NumBins-df,'upper'),'nparams',1,'NBins',h(f).NumBins,'alpha',pAlpha_or_nrMad);
%     [chi_h(f,df),chi_p(f,df),chi_stats{f,df}] = chi2gof(h(f).S_used,'cdf',@(x)chi2cdf(x, h(f).NumBins-df,'upper'),'nparams',1,pAlpha_or_nrMad);
%          figure, plot(chi_stats{f,1}.E,'k.-'), hold on, grid on
%          plot(chi_stats{f,1}.O,'ro-')
%          title(f);drawnow
%          chi_stats{f,1}.O
%          chi_stats{f,1}.E
%         
%         figure, subplot(2,1,1),histogram(h(f).S_used,h(f).NumBins)
%         subplot(2,1,2),histogram(chi2rnd(1,length(h(f).S_used),1),h(f).NumBins)
...
%         pause; close
        ...
        [temp_Values,temp_BinEdges] = histcounts(h(f).S_used_norm,h(f).NumBins,'Normalization','pdf');
        ...
        %[chi_h(f,df),chi_p(f,df),chi_stats{f,df}] = chi2gof(h(f).Values,chiT(f).chi2valsPdf,'nparams',1,'alpha',pAlpha_or_nrMad);
        
        relSSQ(f,1) = sum((h(f).Values - chiT(f).chi2valsPdf).^2) ./ sum((chiT(f).chi2valsPdf));
        chiprobability(f,df) =relSSQ(f,1);
        %chiprobability(f,df) = cdf('Chisquare',relSSQ(f,1),df)
        if plotOrNotIterations == 1
    ...
    if plotOrNot == 1
        ...
        figure('Position',[1   771   640   224]), plot(relSSQ,'o-k')
        ...
        title('Outliers method: Best adjusted observed values vs theoretical chi2 values')
...
%% METHOD 2
    ...
    if plotOrNot == 1
        ...
        title('dMAD method: Scores distribution, median (black line) and threshold (red line)')
        xlabel('Scores values'),ylabel('pdf of scores values')
        ...
        plot((xlim)',S_limit*ones(2,1),'-r')
        title('dMAD method: Scores inside limit in black, outside in red')
        xlabel('Variable index'),ylabel('Scores')
    end
